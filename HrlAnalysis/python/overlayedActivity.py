...
def analyzeData(cellGroup,options,binFiles):
    ...
    if len(rasterInfo.spikes) > 0:
        ...
        ax1.scatter(rasterInfo.time,rasterInfo.spikes,
                    s=options.markersize,marker = [5,3,0],
                    linewidths = '.1', color = 'k')
        #ax1.set_title(cellGroup['name'],size=32)
        ...
        if not options.ignoreRate:
            ax2 = plt.twinx()
            windowRange = range(options.startTime,
                                options.endTime,(options.StepSize))
            ax2.plot(windowRange,rateInfo.rates, color='r',
                     alpha=0.5,linewidth=5)
            ax2.set_ylabel('Mean Rate (Hz)',size=24)
            ax2.axes.set_xlim(options.startTime, options.endTime + 10)
            ax2.grid(False)
