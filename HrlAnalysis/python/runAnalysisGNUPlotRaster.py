...
def analyzeData(cellGroup,options,binFiles):
    ...
    if len(rasterInfo.spikes) > 0:
        g = Gnuplot.Gnuplot()
        d = Gnuplot.Data(rasterInfo.time,rasterInfo.spikes,with_='dots lw 6')
        g.title("HRL Simulator Single Run Tonic: %s" % (cellGroup['name']))
        g('set autoscale')
        g('unset log')
        g('unset label')
        g('set xtic auto')
        ...
        g('set nokey')
        g("set output %s.ps" % (cellGroup['name']))
        strYrange = "[%s:%s]"%(cellGroup['startIdx'],cellGroup['endIdx'])
        g.set_range('yrange',strYrange)
        g.xlabel('Time (s)')
        g.ylabel('Cell Index')
        g.plot(d)
        fileName = '%s.ps' % (cellGroup['name'])
        g.hardcopy(filename=fileName,terminal='postscript',
                   landscape=1,enhanced=1,monochrome=1)
...
def getParser():
    ...
    plot_group.add_argument(
                    ...
                    action="store",default="raster",
                    help="The type of plot to output: options for type are raster: uses the GNUPlot library.", 
                    metavar="TYPE")
